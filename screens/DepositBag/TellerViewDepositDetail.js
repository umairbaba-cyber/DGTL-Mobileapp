...
export default function CustomerViewDepositDetail({ navigation, route }) {
  ...
  console.log('Details',DepositDetail)
    ...
    // const ScannedByTellsser = useSelector(state => route.params.status == 'pending' ?console.log( state.Main.UserDepositRecord.depositsThisMonthPending[route.params.selectedItem].ScannedByTeller): console.log( state.Main.UserDepositRecord.depositsAllTimeApproved[route.params.selectedItem]));
        ...
        return (
            <ScrollView style={{ flex: 1, backgroundColor: '#fff' }}>
                ...
                <View style={styles.monthDeposit}>
                    ...
                    <View style={{width:'45%'}}>
                        <Text style={[styles.monthDepositDescription,{fontWeight:'700'}]}>{customerName}</Text>
                        <Text style={styles.monthDepositDescription}>Acct# {DepositDetail?.customer_details?.[0]?.accountNumber}</Text>
                        <Text style={[styles.monthDepositDescription,{fontWeight:'700'}]}>{new Date(DepositDetail.createdAt).toDateString()}</Text>
                    </View>
                ...
                <View style={{ flex: 1, }}>
                    ...
                    <View style={{ marginHorizontal: responsiveWidth(7) }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginHorizontal: responsiveWidth(1.5) }}>
                            ...
                            <Image style={styles.imageStyle} source={require('../../assets/rightarrow.png')} />
                            <Text style={{ fontSize: responsiveFontSize(2.1), color: '#000', fontWeight: '700', width: responsiveWidth(20) }}>{currencyArray[1]+DepositDetail.total.value.toFixed(2)}</Text>
