...
export default function TellerVerifyDeposit({navigation, route}) {
  ...
  const total = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle.total.value,
  );
  const updatedAt = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle.updatedAt,
  );
  const limitExceed = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle.limitExceed,
  );
  ...
  const tellerLimit = useSelector(
    state => state.Main.User.data?.userData?.tellerLimit,
  );
  const sbt = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle.ScannedByTeller,
  );
  const sbs = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle.ScannedBySupervisor,
  );
  const accountType = useSelector(
    state => state.Main.QrCodeScanedDetail.accoutType,
  );
  ...
  const customerName = useSelector(
    state => state?.Main?.QrCodeScanedDetail?.customer?.name,
  );

  const asa = useSelector(state => state.Main.QrCodeScanedDetail);
  const userId = useSelector(state => state?.Main?.User?.data?.userData?._id);
  const hasDiscrepancy = useSelector(
    state => state.Main.QrCodeScanedDetail?.qrsingle?.discrepancies,
  );
  // const data = route?.params

  console.log('Da>>', hasDiscrepancy);
  // const customerName = data?.item?.customer_details[0]?.name

  const Total = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle.total,
  );
  ...
  const Checks = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle.checks,
  );
  ...
  const accountNumber = useSelector(
    state => state.Main.QrCodeScanedDetail?.customer?.accountNumber,
  );
  const QRSingle = useSelector(
    state => state.Main.QrCodeScanedDetail?.qrsingle,
  );
  const QrCodeScanedDetail = useSelector(
    state => state.Main.QrCodeScanedDetail,
  );
  const {clientID, bagID, customerID} = useSelector(
    state => state.Main.QrCodeScanedDetail.qrsingle,
  );
  ...
  async function SendVerificationRequest() {
    ...
    if (!sbt && !sbs) {
      ...
      if (accountType === 'teller') {
        ...
        ScannedBySecondSupervisor =
          found === 'Yes' ? false : ScannedBySecondSupervisor;
  ...
  return (
    <ScrollView style={{flex: 1, backgroundColor: '#fff'}}>
      ...
      <View style={{flex: 1}}>
                ...
                <View style={styles.discrepancyContainer}>
                  <View style={{flexDirection: 'row'}}>
                    ...
                    <Text
                      style={
                        styles.normalTxt
                      }>{`${QRSingle?.discrepanciesType}`}</Text>
                  </View>
                  <View style={{flexDirection: 'row'}}>
                    ...
                    <Text
                      style={
                        styles.normalTxt
                      }>{`${QRSingle?.discrepanciesAmount}`}</Text>
                  </View>
                </View>
                <Text
                  style={[
          ...
          <>
            ...
            <View style={{marginHorizontal: responsiveWidth(6)}}>
                  ...
                  <View
                    ...
                    <View style={{flexDirection: 'row', alignItems: 'center'}}>
                      <Text style={styles.column2}>
                        {getCoins(item.name, item.value)}
                      </Text>
                      <View style={{width: responsiveWidth(7)}} />
                      <ChangeXcdValues item={item} index={index} />
                    </View>
                  </View>
                );
              })}
              {totalAmountFooter(
                ...
                getTotalAmount(XCD_Without_Zeroes, 'value'),
              )}
            </View>
          </>
        ) : (
                ...
                return (
                  <View
                    ...
                    <View style={{flexDirection: 'row', alignItems: 'center'}}>
                      ...
                      <View style={{width: responsiveWidth(7)}} />
                      <ChangedFX item={item} index={index} />
                    </View>
                  </View>
                );
              })}
        ...
        {Checks.length > 0 ? (
          <>
            ...
            <View style={{marginHorizontal: responsiveWidth(6)}}>
                ...
                return (
                  <View
                    ...
                    <View style={{flexDirection: 'row', alignItems: 'center'}}>
                      ...
                      <View style={{width: responsiveWidth(7)}} />
                      <ChangedChecks item={item} index={index} />
                    </View>
                  </View>
                );
              })}
              {totalAmountFooter(
                'Total Cheques',
                getTotalAmount(Checks, 'checkAmount'),
              )}
            </View>
          </>
        ) : (
        ...
        {/* {accountType === 'teller' && !sbt ? (
          discrepancyView()
        ) : null}

        {accountType === 'supervisor' && !sbt? (
          discrepancyView()
        ) : null} */}
