...
import { StyleSheet, Image, ActivityIndicator, View, ScrollView, TouchableOpacity, Text, } from 'react-native';
...
import { UpdateUserDepositRecord, updateNotificationStatus } from '../../redux/Action';
...
export default function Home({ navigation }) {
    ...
    function LoadTellerHomePageData(token) {
        axios.get(
            ...
            (res) => {
              ...
              console.log('teller home page data',JSON.stringify(res.data))
                ...
                if (code == 200) {
                    ...
                    console.log('pending',data.depositsAllTimePending)
            ...
            alert('Error occured ', e.response.data)
    ...
    if (isLoading) {
        ...
        return (
            <View style={styles.loginContainer}>
                <ScrollView contentContainerStyle={{ paddingHorizontal: responsiveScreenWidth(3) }} style={{ flex: 1 }}>
                    ...
                    <View style={styles.bioContainer}>
                        ...
                        <TouchableOpacity onPress={() =>{ navigation.navigate('Notifications')}}>
                            <Image style={styles.Notification} source={active ? require('../../assets/noti_active.png') :require('../../assets/notified.png') } />
                        </TouchableOpacity>
                    </View>
                    ...
                    {users.accountType == "customer" ?
                        <View style={styles.depositContainer}>
                            ...
                            <TouchableOpacity onPress={() => navigation.navigate('QrCode')}
                             style={styles.depositButtons}>
                                <Text style={[styles.depositText, { fontWeight: 600, fontSize: responsiveFontSize(4.0) }]}>+</Text>
                                <Text style={styles.depositText}>New Deposit</Text>
                                {/* <Text style={styles.depositText}>Bag</Text> */}
                            </TouchableOpacity>
                            {/*  */}
                            <TouchableOpacity onPress={() => navigation.navigate('DepositHistoryMonthly')} style={styles.depositButtons}>
                                <Text style={[styles.depositText, { color: '#8fd150', fontSize: responsiveFontSize(2.4) }]}>{currencyArray[1]+monthlyDeposit.toFixed(2)} </Text>
                        ...
                        <View style={styles.depositContainer}>
                            {/* BarCodeScan and below button is hitting same function because task is same i.e scan code and transfer data to next screen */}
                            <TouchableOpacity onPress={() => navigation.navigate('QrCode')} style={styles.depositButtons}>
                                <Text style={[styles.depositText, { fontWeight: 600, fontSize: responsiveFontSize(4.0) }]}>+</Text>
                                <Text style={styles.depositText}>Process Bag</Text>
                                {/* <Text style={styles.depositText}>Bag</Text> */}
                            </TouchableOpacity>
                            {/*  */}
                            <TouchableOpacity onPress={() => navigation.navigate('DepositHistoryTeller')} style={styles.depositButtons}>
                                <Text style={[styles.depositText, { color: '#8fd150', fontSize: responsiveFontSize(2.4) }]}> {monthlyDeposit} </Text>
                    ...
                    <View style={styles.recentDepostTextConatiner}>
                        {users.accountType == "customer" ? <Text style={styles.RecentDeposit}>Recent Deposits</Text> : <Text style={styles.RecentDeposit}>Pending Deposits</Text>}
                    </View>
                    <View style={{ marginLeft: 5, }}>
                        <RecentDeposits navigation={navigation} Deposits={customerDeposit} role={users.accountType} />
                    </View>
