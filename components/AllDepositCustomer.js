            ...
            renderItem={({ item, index }) => {
                return (
                    <View style={{marginTop: 9}}>
                        <Text style={{ color: '#636363', fontSize: 15}}>{getFormatedDate(item)}</Text>
                        <View style={{ backgroundColor: '#D3D3D3', height: 1, marginTop: 5, marginBottom: 18}}></View>
                        <FlatList
                            data={data[item]}
                            extraData={data[item]}
                              ...
                              console.log('total -----', item);
                                return (
                                    <TouchableOpacity
                                      ...
                                      onPress={() => {
                                        navigation.navigate('CustomerViewDepositDetail', { selectedItem: getIndex(item._id), detail: 'Yearly' });
                                      }}
                                    >
                                        ...
                                        <View style={{ marginLeft: 10 }}>
                                          ...
                                          >
                                            {item.ScannedByTeller && item.ScannedBySupervisor
                                              ...
                                              ? "Un-verified"
                                              : "Pending"
                                            } {moment(item.createdAt).format('YYYY/MM/DD')}, {new Date(item.createdAt).toLocaleTimeString()}
                                          </Text>
                                        </View>
                                      </View>
                                    </TouchableOpacity>
